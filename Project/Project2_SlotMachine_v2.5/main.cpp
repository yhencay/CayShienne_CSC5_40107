 ...
 * File:   main.cpp
 * Author: Shienne Cay
 * Created on February 11, 2017, 7:49 PM
...
int main(int argc, char** argv) {
    ...
    vector <int> spins(SIZE);           //Integer Vector
    ...
    do {
            ...
            switch (begin) {                                            //Case statement for choice in variable Begin
                            ...
                            do {
                                ...
                                if ((choice == 'S' || choice == 's') && budget>minVal) {       //If user chose to spin and budget is greater than 0   
                                    ...
                                    if (play == minVal) {                                             //If total spin count is 0 and user chose not to play game after money inserted
                                        budget -= (cshBon+minCsh);                                             //Deduct cash bonus from inserted amount
                                        bonWin -= (cshBon+minCsh);                                              //Deduct cash bonus added for total record
                                        cout<<"\nNOTICE: Cash bonus is deducted because you have not played yet!\n"
                                            <<"    If you played the mini-game, winnings are also deducted."<<endl<<endl;
                ...
                case 'S': {                 //If user enters staff login access
                    ...
                    if (valPass(input, empName)) {          //If logged in, display game player's information
                        cout<<"\n\tPlayer List \t        Total Spins \t Total Cash \t     Cash Bonus    \t   Cash Won \t      Cash Loss"<<endl;
                        cout<<"\t---------------------------------------------------------------------------------------------------------------"<<endl;
                        for (int cHere = 0; cHere < ROWS; cHere++) {
                            cout<<setw(19);
                            cout<<names[cHere]<<setw(19)<<spins[cHere]<<setw(21);                       //Game can only hold 25 players before it prompts for admin reset
        ...
        if (x == false) {                                   //Start loop exit condition
            ...
            spins[count] = play;                            //Number of spins per player in one game
