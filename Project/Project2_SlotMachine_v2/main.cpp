...
string name();          //Enter name to be used
float insert(float &);              //Insert budget
...
void viewRep(string, int, int, int, float, float, float, float, float, float, float); //Game report from previous 
...
int main(int argc, char** argv) {
    ...
    float lost[SIZE] = {}, won[SIZE] = {}, bonus[SIZE] = {}, cash[SIZE] = {};
    int spins[SIZE] = {};
  ...
  //  do {
    while (v) {
        ...
        cout<<"\nARE YOU A NEW PLAYER OR RETURNING PLAYER? "<<endl;
        cout<<"         N - NEW    R - RETURNING"<<endl<<endl;
        ...
        switch(start) {
            ...
            case 'N': {
                ...
                in.close();
                if (play>0) {
                    cout<<"\nWelcome back, "<<name<<"! You are a returning player!"<<endl<<endl;
                    cout<<"Would you like to see your previous game report?"<<endl;
                    cout<<"      Y - YES    N - NO"<<endl<<endl;
                    cout<<"CHOICE: ";
                    cin>>report;
                    if (report == 'Y' || report == 'y') viewRep(name,play,numWin,numLose,cshIns,win,bonWin,totCash,loss,winPer,lossPer); 
                    w= false;
                }
                else if (play<=0) {
                    names[count] = name;
            ...
            case 'R': {
                ...
                if (play>0) {
                    ...
                    names[count] = name;
                    cout<<"\nYou are a new player. Welcome "<<name<<"!"<<endl<<endl;
                    w=false;
                }                
            } break;
    ...
    if (start == 'R' || start == 'r' || start == 'n' || start =='N') {
        play = 0;       bonWin = 0;     winPer = 0;
        numWin = 0;     win = 0;        loss = 0;
        numLose = 0;    cshIns = 0;     totCash = 0;    lossPer=0;
    do {
...
void viewRep(string name, int play, int numWin, int numLose, float cshIns, float win, 
        float bonWin, float totCash, float loss, float winPer, float lossPer) {
    ifstream in;
    in.open(name+".dat");
    in>>play;                   //File from play
